...
static int prepare_cmd(struct ccxt_device *ccxt,
                       const u8 *command, size_t command_len)
{
...
static int prepare_endpoint_cmd(struct ccxt_device *ccxt,
                                const u8 *command, size_t command_len,
                                u8 endpoint)
{
...
static int send_usb(struct ccxt_device *ccxt)
{
	...
	ret = hid_hw_output_report(ccxt->hdev, ccxt->cmd_buffer, OUT_BUFFER_SIZE);
	if (ret < 0)
		return ret;
...
static int get_fw_version(struct ccxt_device *ccxt)
{
	...
	ccxt->firmware_ver.patch = (u16)ccxt->buffer[5] |
	                          (u16)ccxt->buffer[6] << 8;
...
// TARGET: 00 08 06 01 07 00 00 00 07 00 01 04 00 32 00
...
static int get_fan_cnct(struct ccxt_device *ccxt)
{
	...
	for (channel = 0; channel < min(num_fans, NUM_FANS); channel++) {
		...
		scnprintf(ccxt->fan_label[channel],
		          LABEL_LENGTH,
		          "fan%d", channel + 1);
	}
...
static int get_temp_cnct(struct ccxt_device *ccxt)
{
	...
	// TODO: implement temp sensor connection status
	/*ret = send_usb_cmd(ccxt, CTL_GET_TMP_CNCT, 0, 0, 0);
	if (ret)
		return ret;
...
static int set_pwm(struct ccxt_device *ccxt, int channel, long val)
{
	...
	ret = write_data(ccxt, endpoint_fan_pwm,
	                 data_type_set_speed, sizeof(data_type_set_speed),
	                 speed_cmd, sizeof(speed_cmd));
	if (!ret)
		ccxt->target[channel] = -ENODATA;
...
static int ccxt_read(struct device *dev, enum hwmon_sensor_types type,
...
static int firmware_show(struct seq_file *seqf, void *unused)
{
	...
	seq_printf(seqf, "%d.%d.%d\n",
	           ...
	           ccxt->firmware_ver.minor,
	           ccxt->firmware_ver.patch);
...
static int ccxt_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	ccxt->cmd_buffer = devm_kmalloc(&hdev->dev, OUT_BUFFER_SIZE, GFP_KERNEL);
	if (!ccxt->cmd_buffer)
		return -ENOMEM;
	...
	ccxt->data_buffer = devm_kmalloc(&hdev->dev, IN_BUFFER_SIZE, GFP_KERNEL);
	if (!ccxt->data_buffer)
		return -ENOMEM;
	...
	/* temp and fan connection status only updates when the device is powered on */
